...
public class UtMM {
    ...
    public static int[] propositionCodeHumain(int nbCoups, int lgCode, char[] tabCouleurs) {
        ...
        do {
            ...
            Ut.afficher("Couleur disponnible dans tabCouleur : "); Ut.afficherSL(tabCouleurs);
    ...
    public static int[] reponseHumain(int lgCode) {
        ...
        while (!valide) {
            ...
            // On vérifie que reponseHumain avec la fonction repCorrecte

            if (repCorrecte(reponseHumain, lgCode)) {
                valide = true;
            } else {
    ...
    public static boolean estCompat(int[] cod1, int[][] cod, int[][] rep, int nbCoups, int nbCouleurs) {
        ...
        int i = 0;
        int[] codAComparer = new int[cod[0].length];
        int[] repAComparer = initTab(2, 0);

        while (compact && i < nbCoups) {
            for (int j = 0; j < codAComparer.length; j++) {
                codAComparer[j] = cod[i][j];
            }

            // On verifie que rep de cod1 estCompact avec le cod à la ligne i

            repAComparer = nbBienMalPlaces(cod1, codAComparer, nbCouleurs);

            // Si pas compact

            if (repAComparer[0] != rep[i][0] || repAComparer[1] != rep[i][1]) {
                compact = false;
            }
            i++;
        }
    ...
    public static boolean passeCodeSuivantLexicoCompat(int[] cod1, int[][] cod, int[][] rep, int nbCoups,
            int nbCouleurs) {
        if (passeCodeSuivantLexico(cod1, nbCouleurs) && estCompat(cod1, cod, rep, nbCoups, nbCouleurs)) {
            return true;
        }
    ...
    public static String MatriceToString(int[][] mat) {
        String s = "";
        for (int i = 0; i < mat.length; i++) {
            s += Arrays.toString(mat[i]) + ", ";
        }
        return s;
    }
}